...
import { createMonster, rollEliteTier, setItemsRef } from './monsterFactory'
export { createMonster, rollEliteTier, setItemsRef }
import { buildAbilityStates, CLASS_ABILITIES } from './abilities'
...
import { generateMegaBiome } from './megaBiomes'
import { generateArenaMap } from './arena'
import { generateTempleBlessingsMap, generateInfiniteDungeonMap, generateCelestialMarketMap, generateDragonSanctuaryMap } from './specialDestinations'
...
if (typeof globalThis !== 'undefined') {
  (globalThis as any).__ITEMS__ = ITEMS
}
...
export function makeTile(type: TileType): Tile {
  ...
  if (!cached) {
    ...
    cached = Object.freeze({ type, walkable, transparent: true })
    TILE_CACHE[type] = cached
  }
  return cached
}

const MAP_CACHE: Record<string, GameMap> = {}

export function generateMap(id: string): GameMap {
  if (id === 'unified_world' || id === 'city') {
    return generateUnifiedWorld()
  }

  if (MAP_CACHE[id]) {
    const cached = MAP_CACHE[id]
    return {
      ...cached,
      monsters: cached.monsters.map(m => ({
        ...m,
        isDead: false,
        hp: m.maxHp,
        position: { x: (m as any)._spawnX ?? m.position.x, y: (m as any)._spawnY ?? m.position.y },
      })),
    }
  }
  ...
  // Special Destinations Routing
  if (id === 'temple_blessings') {
    map = generateTempleBlessingsMap()
  } else if (id.startsWith('infinite_dungeon')) {
    const floorNum = parseInt(id.replace('infinite_dungeon_f', '').replace('infinite_dungeon', ''), 10) || 1
    map = generateInfiniteDungeonMap(floorNum)
  } else if (id === 'celestial_market') {
    map = generateCelestialMarketMap()
  } else if (id === 'dragon_sanctuary') {
    map = generateDragonSanctuaryMap()
  }

  // Portal Hall for the 30 Continents
  if (id === 'portal_hall') {
    map = generatePortalHallMap()
  }

  // 30 Continents Routing (continent1 .. continent30)
  if (!map && id.startsWith('continent') && !id.includes('_isl')) {
    map = generateContinentMap(id)
  }

  // 20 Island Deep Dungeon Floors Routing (e.g. continent1_isl2_f4)
  if (!map && id.includes('_isl')) {
    const parts = id.split('_')
    const contId = parts[0]
    const islIdx = parseInt(parts[1]?.replace('isl', '') ?? '1', 10)
    const floorNum = parseInt(parts[2]?.replace('f', '') ?? '1', 10)
    map = generateIslandFloorMap(contId, islIdx, floorNum)
  }

  // 100-Floor Dungeon / Masmorra / Catacombs Routing
  if (id.startsWith('dungeon') || id.startsWith('masmorra') || id.startsWith('catacombs') || id.startsWith('endless')) {
    const floorNum = parseInt(id.replace(/\D/g, ''), 10) || 1
    const theme = id.startsWith('catacombs') ? 'catacombs' : id.startsWith('masmorra') ? 'masmorra' : id.startsWith('endless') ? 'endless' : 'dungeon'
    map = generate100FloorDungeonMap(floorNum, theme)
  }
  ...
  if (!map) {
    ...
    if (biomeDef) {
      let floorNum = 1
      if (id.includes('_f')) {
        floorNum = parseInt(id.split('_f')[1], 10) || 1
      } else {
        const trailingNum = parseInt(id.replace(biomeDef.id, ''), 10)
        if (!isNaN(trailingNum)) floorNum = trailingNum
      }
      map = generate100BiomeMap(biomeDef.id, floorNum)
    }
  }

  if (!map) {
    switch (id) {
      case 'city': map = generateUnifiedWorld(); break
      case 'forest': map = generateForestMap(); break
      case 'dungeon': map = generate100FloorDungeonMap(1); break
      case 'desert': map = generateDesertMap(); break
      case 'swamp': map = generateSwampMap(); break
      case 'tundra': map = generateTundraMap(); break
      case 'volcano': map = generateVolcanoMap(); break
      case 'abyss': map = generateAbyssMap(); break
      case 'deepforest': map = generateDeepForestMap(); break
      case 'arena': map = generateArenaMap(); break
      default: {
        if (id.startsWith('endless')) {
          const f = parseInt(id.replace('endless',''), 10) || 1
          map = generateEndlessTowerMap(f)
          break
        }
        const newMap = generateNewBiome(id)
        if (newMap) { map = newMap; break }
        const mega = generateMegaBiome(id)
        if (mega) { map = mega; break }
        const c2 = generateC2Biome(id)
        if (c2) { map = c2; break }
        const extra = generateExtraMap(id)
        if (extra) { map = extra; break }
        map = generateUnifiedWorld()
        break
      }
    }
  }
  ...
  if (map) {
    map = ensureMapAccessibility(map)
    if (!id.startsWith('endless')) {
      MAP_CACHE[id] = map
    }
  }
...
function generateCityMap(): GameMap {
  ...
  const placeHouse = (hx: number, hy: number, w: number, h: number) => {
    ...
    const doorY = hy + h - 1
    if (doorX > 0 && doorX < W - 1 && doorY > 0 && doorY < H - 1 && tiles[doorY]?.[doorX]) {
      tiles[doorY][doorX] = makeTile('house_door')
    }
  }

  // Fewer houses, distributed in 4 districts within W=200, H=200
  for (let district = 0; district < 4; district++) {
    const startX = district % 2 === 0 ? 30 : 110
    const startY = district < 2 ? 30 : 110
    const endX = district % 2 === 0 ? 90 : 170
    const endY = district < 2 ? 90 : 170
    for (let i = 0; i < 5; i++) {
      const w = 8 + Math.floor(Math.random() * 8)
      const h = 6 + Math.floor(Math.random() * 6)
      const x = startX + Math.floor(Math.random() * Math.max(1, endX - startX - w))
      const y = startY + Math.floor(Math.random() * Math.max(1, endY - startY - h))
      placeHouse(x, y, w, h)
    }
  }
  ...
  // Market areas
  const marketAreas = [
    { x: 70, y: 70 }, { x: 130, y: 70 }, { x: 70, y: 130 }, { x: 130, y: 130 },
  ]
  for (const { x: mx, y: my } of marketAreas) {
    for (let dy = -3; dy <= 3; dy++) {
      for (let dx = -3; dx <= 3; dx++) {
        if (Math.random() < 0.3 && tiles[my + dy]?.[mx + dx]) {
          tiles[my + dy][mx + dx] = makeTile('market_stall')
        }
      }
    }
  }
  ...
  const portalLocations: { x: number; y: number; name: string }[] = [
    { x: 80,  y: 180, name: 'Desert' },
    { x: 120, y: 180, name: 'Swamp' },
    { x: 160, y: 180, name: 'Tundra' },
    { x: 180, y: 40,  name: 'Dungeon' },
    { x: 180, y: 80,  name: 'Volcano' },
    { x: 180, y: 120, name: 'Crystal Cave' },
    { x: 180, y: 160, name: 'Haunted Ruins' },
    { x: 20,  y: 40,  name: 'Sky Realm' },
    { x: 20,  y: 100, name: 'Deep Forest' },
    { x: 20,  y: 160, name: 'Abyss' },
    { x: 40,  y: 15,  name: 'Pradaria Florida' },
    { x: 160, y: 15,  name: 'Costa Rochosa' },
    { x: 100, y: 120, name: 'Catacumbas' },
    { x: 25,  y: 25,  name: 'Montanha Nevada' },
    { x: 175, y: 175, name: 'Ruinas Antigas' },
    { x: 20,  y: 180, name: 'Bosque de Cristal' },
    { x: 100, y: 180, name: 'Savana Dourada' },
    { x: 180, y: 100, name: 'Arquipelago Mistico' },
    { x: 15,  y: 15,  name: 'Vale Esquecido' },
    { x: 80,  y: 15,  name: 'Fenda Estelar' },
    { x: 120, y: 15,  name: 'Jardim Eterno' },
    { x: 100, y: 85,  name: 'Cidade 2' },
    { x: 100, y: 70,  name: 'Torre Infinita' },
    { x: 108, y: 100, name: 'Arena do Coliseu' },
    { x: 92,  y: 100, name: 'Dungeon (Spawn)' },
  ]
  ...
  const fillBand = (
    ...
    for (let y = Math.max(0, yStart); y < Math.min(newH, yEnd); y++) {
      for (let x = 0; x < W; x++) {
        let t: TileType = base
  ...
  fillBand(0,  12, 'grass',     [{ tile: 'flower', chance: 0.12 }, { tile: 'tall_grass', chance: 0.06 }])
  fillBand(12, 24, 'tall_grass',[{ tile: 'tree',   chance: 0.10 }, { tile: 'flower',     chance: 0.05 }])
  fillBand(24, 36, 'dirt',      [{ tile: 'rock',   chance: 0.08 }, { tile: 'grass',      chance: 0.20 }])
  fillBand(36, 48, 'stone',     [{ tile: 'rock',   chance: 0.12 }, { tile: 'dirt',       chance: 0.20 }])
  fillBand(48, 60, 'tall_grass',[{ tile: 'tree',   chance: 0.22 }, { tile: 'rock',       chance: 0.06 }])

  const sStart = NORTH + H
  fillBand(sStart,      sStart + 12, 'sand',      [{ tile: 'flower', chance: 0.05 }, { tile: 'grass', chance: 0.10 }])
  fillBand(sStart + 12, sStart + 24, 'sand',      [{ tile: 'rock',   chance: 0.10 }, { tile: 'water', chance: 0.08 }])
  fillBand(sStart + 24, sStart + 36, 'water',     [{ tile: 'sand',   chance: 0.18 }, { tile: 'rock',  chance: 0.05 }])
  fillBand(sStart + 36, sStart + 48, 'deepwater', [{ tile: 'water',  chance: 0.20 }, { tile: 'rock',  chance: 0.03 }])
  fillBand(sStart + 48, sStart + 60, 'deepwater', [{ tile: 'water',  chance: 0.06 }])
  ...
  // PORTOES
  const gates = [50, 100, 150]
  for (const gx of gates) {
    for (let dx = -3; dx <= 3; dx++) {
      if (gx + dx <= 0 || gx + dx >= W - 1) continue
  ...
  const addBandMobs = (
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        ...
        my = yStart + Math.floor(Math.random() * Math.max(1, yEnd - yStart))
  ...
  addBandMobs(48, 60, ['slime', 'wolf'], 2, 4, 6)
  addBandMobs(36, 48, ['goblin', 'spider'], 5, 7, 6)
  addBandMobs(24, 36, ['orc', 'skeleton'], 8, 11, 5)
  addBandMobs(12, 24, ['troll', 'zombie'], 12, 15, 4)
  addBandMobs(0,  12, ['treant', 'witch'], 16, 20, 3, 'elite')

  addBandMobs(sStart,      sStart + 12, ['goblin', 'slime'], 3, 5, 6)
  addBandMobs(sStart + 12, sStart + 24, ['spider', 'skeleton'], 6, 9, 6)
  addBandMobs(sStart + 24, sStart + 36, ['zombie', 'orc'], 10, 13, 5)
  addBandMobs(sStart + 36, sStart + 48, ['ghost', 'witch'], 14, 18, 4, 'elite')
  addBandMobs(sStart + 48, sStart + 60, ['vampire', 'demon'], 19, 25, 3, 'champion')

  const WEST_BIOME_COUNT = 10
  const WEST_BIOME_W = 15
  const WEST = WEST_BIOME_COUNT * WEST_BIOME_W
  const EAST = 60
  ...
  const eastGates = [50, 100, 150].map(r => r + NORTH)
  ...
  for (let b = 0; b < WEST_BIOME_COUNT; b++) {
      ...
      for (const dx of doorXs) {
        for (const dy of doorYs) {
          for (let oy = -2; oy <= 2; oy++) {
            if (expTiles[dy + oy]) expTiles[dy + oy][dx] = makeTile('cobblestone')
          }
        }
      }
    }
    ...
    for (let x = x0 + 1; x < x1; x++) {
      if (expTiles[floorMidY]) expTiles[floorMidY][x] = makeTile('wall')
      if (expTiles[floorBotY]) expTiles[floorBotY][x] = makeTile('wall')
    }
    // portas entre andares
    const gateX = x0 + Math.floor(WEST_BIOME_W / 2)
    for (let gd = -3; gd <= 3; gd++) {
      if (expTiles[floorMidY] && expTiles[floorMidY][gateX + gd]) expTiles[floorMidY][gateX + gd] = makeTile('cobblestone')
      if (expTiles[floorBotY] && expTiles[floorBotY][gateX + gd]) expTiles[floorBotY][gateX + gd] = makeTile('cobblestone')
    }
    ...
    for (const f of floors) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (!expTiles[my] || !expTiles[my][mx] || !expTiles[my][mx].walkable) continue
  ...
  const westGates = [50, 100, 150].map(r => r + NORTH)
...
function generateForestMap(): GameMap {
  ...
  for (let x = 0; x < W; x++) {
    if (tiles[Math.floor(H/4)]?.[x]?.walkable) tiles[Math.floor(H/4)][x] = makeTile('road')
    if (tiles[Math.floor(H/2)]?.[x]?.walkable) tiles[Math.floor(H/2)][x] = makeTile('road')
    if (tiles[Math.floor(3*H/4)]?.[x]?.walkable) tiles[Math.floor(3*H/4)][x] = makeTile('road')
  }
  for (let y = 0; y < H; y++) {
    if (tiles[y]?.[Math.floor(W/4)]?.walkable) tiles[y][Math.floor(W/4)] = makeTile('road')
    if (tiles[y]?.[Math.floor(W/2)]?.walkable) tiles[y][Math.floor(W/2)] = makeTile('road')
    if (tiles[y]?.[Math.floor(3*W/4)]?.walkable) tiles[y][Math.floor(3*W/4)] = makeTile('road')
  }

  // Portal at city entrance
  tiles[20][20] = makeTile('portal')
  // Portal to deep forest
  tiles[180][180] = makeTile('portal')

  const monsters: Monster[] = []
  const spawnGroups: { type: MonsterType; count: number; minLvl: number; maxLvl: number }[] = [
    { type: 'slime', count: 15, minLvl: 2, maxLvl: 8 },
    { type: 'wolf', count: 12, minLvl: 3, maxLvl: 10 },
    { type: 'goblin', count: 10, minLvl: 2, maxLvl: 8 },
    { type: 'spider', count: 8, minLvl: 3, maxLvl: 10 },
    { type: 'skeleton', count: 6, minLvl: 4, maxLvl: 12 },
    { type: 'troll', count: 4, minLvl: 5, maxLvl: 15 },
    { type: 'witch', count: 3, minLvl: 8, maxLvl: 18 },
  ]

  for (const group of spawnGroups) {
    for (let i = 0; i < group.count; i++) {
      const px = 20 + Math.floor(Math.random() * 160)
      const py = 20 + Math.floor(Math.random() * 160)
...
// ─── Player Factory ─────────────────────────────────────────────────────────
import { ALL_100_CLASSES, CLASS_META_MAP } from './hundredClassesData'

export const BASE_STATS: Record<string, CharacterStats> = {}
export const START_WEAPONS: Record<string, string> = {}
const PRIMARY_SKILL: Record<string, { name: string; icon: string }> = {}

ALL_100_CLASSES.forEach(c => {
  BASE_STATS[c.id] = c.baseStats
  START_WEAPONS[c.id] = c.startWeapon
  PRIMARY_SKILL[c.id] = { name: c.primarySkillName, icon: c.primarySkillIcon }
})

function makeClassProgress(cls: string): import('./types').ClassProgress {
  const meta = CLASS_META_MAP[cls] || ALL_100_CLASSES[0]
  const pSkill = PRIMARY_SKILL[cls] || { name: 'Ataque', icon: '⚔️' }
  const startWepKey = START_WEAPONS[cls] || 'wooden_sword'
  return {
    level: 1,
    xp: 0,
    xpToNext: 100,
    skills: [
      { name: pSkill.name, level: 1, xp: 0, xpToNext: 50, icon: pSkill.icon },
      { name: 'Defesa', level: 1, xp: 0, xpToNext: 50, icon: '🛡️' },
      { name: 'Vitalidade', level: 1, xp: 0, xpToNext: 50, icon: '❤️' },
    ],
    abilities: buildAbilityStates(cls as CharacterClass),
    equipment: {
      weapon: ITEMS[startWepKey] ? { ...ITEMS[startWepKey] } : { ...ITEMS.wooden_sword },
      armor: null, helmet: null, boots: null, ring: null,
    },
  }
}

export function createPlayer(name: string, cls: CharacterClass = 'knight'): Player {
  const meta = CLASS_META_MAP[cls] || ALL_100_CLASSES[0]
  const stats = { ...(BASE_STATS[cls] || meta.baseStats) }
  const inventory = Array(30).fill(null) as (Item | null)[]
  inventory[0] = { ...ITEMS.small_potion, quantity: 10 }
  inventory[1] = { ...ITEMS.mana_potion, quantity: 10 }
  const startWepKey = START_WEAPONS[cls] || 'wooden_sword'
  inventory[2] = ITEMS[startWepKey] ? { ...ITEMS[startWepKey] } : { ...ITEMS.wooden_sword }

  const classProgress: Record<string, import('./types').ClassProgress> = {}
  ALL_100_CLASSES.forEach(c => {
    classProgress[c.id] = makeClassProgress(c.id)
  })

  const classAbilities = CLASS_ABILITIES[cls] || []
  const equippedAbilities = classAbilities.slice(0, 4)
  ...
  return {
    ...
    skills: classProgress[cls].skills,
    abilities: classProgress[cls].abilities,
    equippedAbilities,
    subskills: {},
    classProgress,
    buffs: [],
...
export function recalcStats(player: Player): CharacterStats {
  const base = player.baseStats ? { ...player.baseStats } : { ...BASE_STATS[player.class || 'knight'] }
  ...
  const lvlBonus = (Math.max(1, player.level || 1) - 1) * 0.12
...
export function sanitizePlayer(p: any, defaultClass: CharacterClass = 'knight'): Player {
  const cls = (p?.class || defaultClass) as CharacterClass
  const baseDefault = createPlayer(p?.name || 'Guerreiro', cls)
  if (!p || typeof p !== 'object') return baseDefault

  const eq = p.equipment || {}
  const sanitizedEquipment = {
    weapon: eq.weapon ?? baseDefault.equipment.weapon,
    armor: eq.armor ?? null,
  ...
  const posX = p.position && Number.isFinite(p.position.x) ? p.position.x : baseDefault.position.x
  const posY = p.position && Number.isFinite(p.position.y) ? p.position.y : baseDefault.position.y

  const sanitizedPlayer: Player = {
    ...baseDefault,
    ...p,
    name: p.name || baseDefault.name,
    class: cls,
    level: Number.isFinite(p.level) && p.level >= 1 ? Math.floor(p.level) : 1,
    xp: Number.isFinite(p.xp) && p.xp >= 0 ? Math.floor(p.xp) : 0,
    xpToNext: Number.isFinite(p.xpToNext) && p.xpToNext >= 10 ? Math.floor(p.xpToNext) : 100,
    hp: Number.isFinite(p.hp) && p.hp >= 1 ? Math.floor(p.hp) : baseDefault.hp,
    mp: Number.isFinite(p.mp) && p.mp >= 0 ? Math.floor(p.mp) : baseDefault.mp,
    gold: Number.isFinite(p.gold) && p.gold >= 0 ? Math.floor(p.gold) : 0,
    position: { x: posX, y: posY },
    direction: p.direction || 'down',
    equipment: sanitizedEquipment,
    inventory: Array.isArray(p.inventory) ? p.inventory : baseDefault.inventory,
    skills: Array.isArray(p.skills) && p.skills.length > 0 ? p.skills : baseDefault.skills,
    abilities: Array.isArray(p.abilities) && p.abilities.length > 0 ? p.abilities : baseDefault.abilities,
    classProgress: p.classProgress || baseDefault.classProgress,
    buffs: Array.isArray(p.buffs) ? p.buffs : [],
    reputation: p.reputation || createDefaultReputation(),
    masteries: p.masteries || createDefaultMasteries(),
    pets: p.pets || createDefaultPets(),
    achievements: p.achievements || createDefaultAchievements(),
    prestige: p.prestige || createDefaultPrestige(),
    specializations: p.specializations || createDefaultSpecializations(),
  }

  try {
    sanitizedPlayer.stats = recalcStats(sanitizedPlayer)
  } catch (err) {
    console.warn('[sanitizePlayer] recalcStats error, using defaults', err)
    sanitizedPlayer.stats = { ...baseDefault.stats }
  }
  if (!sanitizedPlayer.baseStats) sanitizedPlayer.baseStats = { ...baseDefault.baseStats }
