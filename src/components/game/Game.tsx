...
import { createPlayer, generateMap, sanitizePlayer } from '@/lib/game/data'
...
import { MercenariesModal } from './MercenariesModal'
import { ShipCraftingModal } from './ShipCraftingModal'
import { Continents30WorldMapModal } from './Continents30WorldMapModal'
...
function buildInitialState(playerName: string, cls: CharacterClass, mapId = 'unified_world', skin = 0): GameState {
  const map = generateMap(mapId)
  if (!map.spawnPoints) {
    map.spawnPoints = [(map as any).playerSpawn || { x: 160, y: 160 }]
  }
  const rawSpawn = map.spawnPoints?.[0] || (map as any).playerSpawn || { x: 160, y: 160 }
  const maxX = Math.max(32, (map.width - 2) * 32)
  const maxY = Math.max(32, (map.height - 2) * 32)
  const spawn = {
    x: Math.max(32, Math.min(maxX, rawSpawn.x || 160)),
    y: Math.max(32, Math.min(maxY, rawSpawn.y || 160)),
  }
...
export default function Game() {
  ...
  const [showMercenaries, setShowMercenaries] = useState(false)
    ...
    if (account && gameState.player) {
      startAutoSave(
        () => ({
          ...
          playtime: Math.floor(gameState.tick / 60), // Convert ticks to seconds
          saveData: JSON.stringify({
            player: gameState.player,
            mapId: gameState.currentMap?.id || 'city',
            version: 1,
          }),
        }),
        false, // not guest
  ...
  useEffect(() => {
    const onDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'KeyK' && !stateRef.current.editorOpen) {
  ...
  // ── Handlers do jogo ──────────────────────────────────────────────────────
  const runLoadingSequence = useCallback((statusMsg: string, onDone: () => void) => {
    setIsMapLoading(true)
    setLoadingProgress(25)
    setLoadingStatus(statusMsg || 'Iniciando Motor Rucoy Offline...')
    ...
    const interval = setInterval(() => {
      ...
      if (current >= 100) {
        ...
        setLoadingStatus('Mundo Pronto! Entrando no Jogo...')
        
        try {
          onDone()
        } catch (e) {
          console.error('[Game] Error entering game world:', e)
        } finally {
          setTimeout(() => {
        ...
        if (current >= 50 && current < 80) {
          setLoadingStatus('Sincronizando Biomas e Terreno...')
        } else if (current >= 80) {
          setLoadingStatus('Renderizando Canvas & Iluminação...')
        }
      }
    }, 35)
  }, [])
  ...
  const handleStart = useCallback((name: string, cls: CharacterClass, skin: number = 0) => {
    const pName = name && name.trim() ? name.trim() : 'Guerreiro'
    setIsMapLoading(true)
    setLoadingProgress(5)
    setLoadingStatus(`Gerando Continente de Biomas para ${pName}...`)

    setTimeout(() => {
      try {
        const newState = buildInitialState(pName, cls, 'city', skin)
        runLoadingSequence(`Sincronizando Mundo Rucoy para ${pName}...`, () => {
          setGameState(newState)
          setShowInventory(false)
        })
      } catch (err) {
        console.error('[Game] Error building initial state:', err)
        try {
          const fallbackState = buildInitialState('Guerreiro', 'knight', 'city', 0)
          setGameState(fallbackState)
        } catch (e2) {
          console.error('[Game] Fallback state error:', e2)
        } finally {
          setIsMapLoading(false)
        }
      }
  ...
  const handleLoad = useCallback((): boolean => {
    try {
      ...
      setIsMapLoading(true)
      setLoadingProgress(5)
      setLoadingStatus('Carregando Progresso do Jogador & Biomas...')

      setTimeout(() => {
        try {
          const sanitizedPlayer = sanitizePlayer(save.player)
          const targetMapId = save.mapId || 'city'
          const map = generateMap(targetMapId)
          if (!map.spawnPoints || map.spawnPoints.length === 0) {
            map.spawnPoints = [(map as any).playerSpawn || { x: 3200, y: 3200 }]
          }
          const citySpawn = map.spawnPoints[0] || { x: 3200, y: 3200 }
          const spawnX = (sanitizedPlayer.position?.x && sanitizedPlayer.position.x > 64 && sanitizedPlayer.position.x < map.width * 32 - 64)
            ? sanitizedPlayer.position.x
            : citySpawn.x
          const spawnY = (sanitizedPlayer.position?.y && sanitizedPlayer.position.y > 64 && sanitizedPlayer.position.y < map.height * 32 - 64)
            ? sanitizedPlayer.position.y
            : citySpawn.y

          runLoadingSequence('Sincronizando Save & Biomas...', () => {
            setGameState({
              screen: 'playing',
              player: { ...sanitizedPlayer, position: { x: spawnX, y: spawnY } },
              currentMap: map,
              camera: { x: Math.max(0, spawnX - 400), y: Math.max(0, spawnY - 270) },
              tick: 0,
              damageNumbers: [],
              particles: [],
              minions: [],
          ...
          console.error('[Game] Error applying loaded game state:', err)
          setIsMapLoading(false)
        }
      }, 30)
      ...
      console.error('[Game] Error in handleLoad:', e)
      setIsMapLoading(false)
      return false
    }
  }, [runLoadingSequence])
  ...
  const handleMapChange = useCallback((mapId: string) => {
    runLoadingSequence(`Viajando para o Bioma: ${mapId.toUpperCase()}...`, () => {
      setGameState((prev) => {
        if (!prev.player) return prev
        ...
        return {
          ...
          chatMessages: [
            ...prev.chatMessages,
            { id: `map_${Date.now()}`, text: `Chegou em: ${newState.currentMap?.name || mapId}`, type: 'system', timestamp: Date.now() },
          ],
          tick: 0,
          editorState: buildInitialEditorState(),
        }
    ...
    <div className="w-full h-full relative overflow-hidden" style={{ background: '#080a0e' }}>
      ...
      {!gameState.editorOpen && gameState.player && gameState.currentMap && !isDead && (
        <RucoyModalBar
          ...
          isAmbitiousOpen={showAmbitious}
          isMercenariesOpen={showMercenaries}
          isShipOpen={showShip}
          isContinentsOpen={showContinents}
          isSubskillsOpen={showSubskills}
          ...
          onToggleAmbitious={() => setShowAmbitious(v => !v)}
          onToggleMercenaries={() => setShowMercenaries(v => !v)}
          onToggleShip={() => setShowShip(v => !v)}
          onToggleContinents={() => setShowContinents(v => !v)}
          onToggleSubskills={() => setShowSubskills(v => !v)}
          ...
          onToggleSkillTree={() => setShowSkillTree(v => !v)}
      ...
      {/* Modal 100 Recursos Ambiciosos */}
      <Ambitious100FeaturesModal
        isOpen={showAmbitious}
        onClose={() => setShowAmbitious(false)}
      />

      {/* Mercenaries Modal */}
      {showMercenaries && !gameState.editorOpen && gameState.player && (
        <MercenariesModal
          player={gameState.player}
          onUpdatePlayer={(p) => setGameState(prev => ({ ...prev, player: p }))}
          onClose={() => setShowMercenaries(false)}
        />
      )}

      {/* Ship Crafting & Navigation Modal */}
      {showShip && !gameState.editorOpen && gameState.player && (
        <ShipCraftingModal
          player={gameState.player}
          onUpdatePlayer={(p) => setGameState(prev => ({ ...prev, player: p }))}
      ...
      {/* 30 Continents World Map Modal */}
      {showContinents && !gameState.editorOpen && gameState.player && (
        <Continents30WorldMapModal
          player={gameState.player}
          onWarpToMap={(mapId) => handleMapChange(mapId)}
          onClose={() => setShowContinents(false)}
        />
      )}
      ...
      {!gameState.editorOpen && gameState.player && gameState.currentMap && gameState.screen === 'playing' && !isDead && (
        <MiniMap
          gameState={gameState}
          onToggleWorldMap={() => setShowRucoyWorld(v => !v)}
        />
      )}
      ...
      {/* Skill Tree Modal */}
      {showSkillTree && !gameState.editorOpen && gameState.player && (
        <SkillTreeModal
          player={gameState.player}
          onClose={() => setShowSkillTree(false)}
          onUpdatePlayer={(updater) => setGameState(prev => prev.player ? { ...prev, player: updater(prev.player) } : prev)}
        />
      )}
      ...
      {showAccount && !gameState.editorOpen && account && (
        <AccountPanel
          ...
          onLoadSave={(slotId) => {
            ...
            if (slot) {
              try {
                localStorage.setItem('rucoy_save', typeof slot.saveData === 'string' ? slot.saveData : JSON.stringify(slot.saveData))
                handleLoad()
              } catch (e) {
      ...
      {/* Subskills Modal */}
      {showSubskills && !gameState.editorOpen && gameState.player && (
        <SubskillsModal
          player={gameState.player}
          onClose={() => setShowSubskills(false)}
          onUpdateLoadout={(equipped, subskills) => {
            setGameState((prev) => {
              if (!prev.player) return prev
              return {
                ...prev,
      ...
      <GameLoader
        ...
        onComplete={() => setIsMapLoading(false)}
      />
