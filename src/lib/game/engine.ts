...
import { calculateXpToNext, recalcStats, ITEMS, generateMap, BASE_STATS, START_WEAPONS, sanitizePlayer } from './data'
...
import { updateMercenariesAI } from './mercenariesSystem'
...
export function damageMonster(
  ...
  // Apply combo multiplier and active blessing
  let finalValue = value
  let combo = state._combo ?? createCombo()
  if (source === 'player') {
    combo = addHit(combo, state.tick)
    const blessAtkMul = (state.player as any)?._activeBlessing?.type === 'damage' ? 1.3 : 1.0
    finalValue = Math.round(value * combo.multiplier * blessAtkMul)
  }
  ...
  if (newHp <= 0) {
    ...
    if (!wasAlreadyDead) {
      ...
      const blessXpMul = (newPlayer as any)._activeBlessing?.type === 'xp' ? 1.4 : 1.0
      const blessGoldMul = (newPlayer as any)._activeBlessing?.type === 'gold' ? 1.5 : 1.0
      const xpGain   = Math.round(monster.xpReward * petBonus.xpMul * blessXpMul)
      const goldGain = Math.round(monster.goldReward * petBonus.goldMul * blessGoldMul)
...
export function updateMonsterAI(state: GameState): GameState {
  ...
  const rawMonsters = map.monsters || []
  const monsters = rawMonsters.map((m, idx) => {
    if (m.isDead) {
      ...
      if (nm.deathTimer <= 0) {
        ...
        if (r <= 0) {
          ...
          if (distToSpawn < 1200) {
            nm = { ...nm, _respawnIn: 1800, _respawnStartTick: tick }  // 30 seconds respawn (1800 ticks @ 60fps)
          } else {
...
function findPath(map: GameMap, startX: number, startY: number, goalX: number, goalY: number): Vec2[] {
  ...
  const isWalkable = (tx: number, ty: number): boolean => {
    ...
    return map.tiles[ty]?.[tx]?.walkable ?? false
  }
...
export function movePlayerTowardTarget(state: GameState): GameState {
  ...
  const isWalkable = (px: number, py: number): boolean => {
    ...
    if ((player.isSailing || player.hasShip) && (tile.type === 'water' || tile.type === 'deepwater')) return true
...
export function getPortalTargetForLocation(
  mapId: string,
  ptx: number,
  pty: number,
  stepTile: string,
  ...
  // 1) On Open World Map (unified_world or city)
  if (mapId === 'unified_world' || mapId === 'city') {
    const distFromCenter = Math.hypot(ptx - CENTER, pty - CENTER)

    // Capital Real Center Portals (Radius 28 around Center)
    if (distFromCenter <= 28) {
      // 1. Check Exact Special Destination Coordinates in Capital Real
      if (Math.abs(ptx - (CENTER - 8)) <= 2 && Math.abs(pty - CENTER) <= 2) return 'temple_blessings'
      if (Math.abs(ptx - CENTER) <= 2 && Math.abs(pty - (CENTER - 8)) <= 2) return 'infinite_dungeon_f1'
      if (Math.abs(ptx - CENTER) <= 2 && Math.abs(pty - (CENTER + 8)) <= 2) return 'arena'
      if (Math.abs(ptx - (CENTER + 8)) <= 2 && Math.abs(pty - CENTER) <= 2) return 'celestial_market'
      if (Math.abs(ptx - (CENTER - 6)) <= 2 && Math.abs(pty - (CENTER - 6)) <= 2) return 'dragon_sanctuary'
      if (Math.abs(ptx - (CENTER + 6)) <= 2 && Math.abs(pty - (CENTER - 6)) <= 2) return 'portal_hall'
      if (Math.abs(ptx - (CENTER - 6)) <= 2 && Math.abs(pty - (CENTER + 6)) <= 2) return 'catacombs1'
      if (Math.abs(ptx - (CENTER + 6)) <= 2 && Math.abs(pty - (CENTER + 6)) <= 2) return 'masmorra1'
      if (Math.abs(ptx - (CENTER - 3)) <= 2 && Math.abs(pty - CENTER) <= 2) return 'dungeon1'
      if (Math.abs(ptx - (CENTER + 3)) <= 2 && Math.abs(pty - CENTER) <= 2) return 'endless1'
      ...
      const plazaTargets: Record<string, string> = {
        '252,240': 'b11_golden_dunes',
        '248,248': 'b21_magma_crater',
        '240,252': 'b41_poison_bog',
        '232,248': 'b61_void_abyss',
        '228,240': 'b31_frostbite_pass',
        '232,232': 'b81_cloud_haven',
        '240,228': 'b1_whispering_woods',
        '248,232': 'b91_coral_reef_coast',
      }
      for (const [key, target] of Object.entries(plazaTargets)) {
      ...
      if (stepTile === 'desert_portal') return 'b11_golden_dunes'
      if (stepTile === 'volcano_portal') return 'b21_magma_crater'
      if (stepTile === 'abyss_portal') return 'b61_void_abyss'
      if (stepTile === 'mountain_portal') return 'b31_frostbite_pass'
      if (stepTile === 'sky_portal') return 'b81_cloud_haven'
      if (stepTile === 'ocean_portal') return 'b91_coral_reef_coast'
      if (stepTile === 'forest_portal') {
        return pty > CENTER ? 'b41_poison_bog' : 'b1_whispering_woods'
      }
      if (stepTile === 'portal') {
        if (ptx > CENTER + 4) return 'b11_golden_dunes'
        if (ptx < CENTER - 4) return 'b31_frostbite_pass'
        if (pty > CENTER + 4) return 'b41_poison_bog'
        if (pty < CENTER - 4) return 'b1_whispering_woods'
        return 'b1_whispering_woods'
      }
    }

    // Biome Stairs/Portals Anywhere on Open World Map -> Find Closest Biome Region
    let closestRegion: any = null
    let minDistance = 9999
    for (const reg of OPEN_WORLD_REGIONS) {
      if (reg.id === 'city') continue
      const d = Math.hypot(ptx - reg.centerX, pty - reg.centerY)
      if (d < minDistance) {
        minDistance = d
        closestRegion = reg
      }
  ...
  // 1.5) Special Destination Return Handlers
  if (mapId === 'temple_blessings' || mapId === 'celestial_market' || mapId === 'dragon_sanctuary' || mapId === 'arena') {
    return 'unified_world'
  }

  if (mapId.startsWith('infinite_dungeon')) {
    const floorNum = parseInt(mapId.replace('infinite_dungeon_f', '').replace('infinite_dungeon', ''), 10) || 1
    let spawnTx = 6, spawnTy = 6
    if (map?.spawnPoints && map.spawnPoints.length > 0) {
      spawnTx = Math.floor(map.spawnPoints[0].x / 32)
      spawnTy = Math.floor(map.spawnPoints[0].y / 32)
    }
    const distToSpawn = Math.hypot(ptx - spawnTx, pty - spawnTy)
    if (distToSpawn <= 6 || stepTile === 'crystal_portal' || stepTile === 'stairs_up') {
      return 'unified_world'
    } else {
      return `infinite_dungeon_f${floorNum + 1}`
    }
  }
  ...
  if (mapId === 'city2') {
    for (const p of CITY2_PORTAL_COORDS) {
      if (Math.abs(ptx - p.x) <= 3 && Math.abs(pty - p.y) <= 3) {
        return `c2_${p.biome}_1`
      }
    }
    if (Math.abs(ptx - CITY2_CX) <= 3 && Math.abs(pty - (CITY2_CY - 6)) <= 3) {
      return 'unified_world'
    }
    return 'unified_world'
  }

  // 2.5) Salão Imperial dos 30 Portais (portal_hall)
  if (mapId === 'portal_hall') {
    if (stepTile === 'stairs_up' || Math.hypot(ptx - 22, pty - 22) <= 2) {
      return 'unified_world'
    }
    if (stepTile === 'portal') {
      const angle = Math.atan2(pty - 22, ptx - 22)
      const normAngle = angle < 0 ? angle + Math.PI * 2 : angle
      const portIdx = Math.floor((normAngle / (Math.PI * 2)) * 30) % 30 + 1
      return `continent${portIdx}`
    }
    return 'unified_world'
  }

  // 2.6) Maps de Continente (continent1 .. continent30)
  if (mapId.startsWith('continent') && !mapId.includes('_isl')) {
    if (stepTile === 'sky_portal' || Math.hypot(ptx - 160, pty - 160) <= 4) {
      return 'portal_hall'
    }
    if (stepTile === 'dungeon_portal' || stepTile === 'haunted_portal' || stepTile === 'portal') {
      // Find nearest island on this continent
      const contNum = parseInt(mapId.replace('continent', ''), 10) || 1
      const islIdx = Math.min(20, Math.max(1, Math.floor((ptx + pty) % 20) + 1))
      return `${mapId}_isl${islIdx}_f1`
    }
  }

  // 3) Inside Submaps / Biomes / Dungeons (Floors 1..10 or 1..100)

  // Determine if player is near spawn point on current map
  let spawnTx = 12
  let spawnTy = 12
  if (map?.spawnPoints && map.spawnPoints.length > 0) {
    spawnTx = Math.floor(map.spawnPoints[0].x / 32)
  ...
  const distToSpawn = Math.hypot(ptx - spawnTx, pty - spawnTy)
  const isNearSpawn = distToSpawn <= 10

  // Determine direction:
  // - stairs_up or rune_altar -> ALWAYS FORWARD
  // - stairs_down -> ALWAYS BACKWARD
  // - portal tiles: near spawn -> BACKWARD (return), far from spawn -> FORWARD (next floor)
  let isForward = false
  if (stepTile === 'stairs_up' || stepTile === 'rune_altar') {
    isForward = true
  } else if (stepTile === 'stairs_down') {
    isForward = false
  } else {
    isForward = !isNearSpawn
  }
  ...
  if (isForward) {
    ...
    if (c2Match) {
      const base = `c2_${c2Match[1]}`
      const floor = parseInt(c2Match[2], 10) || 1
      if (floor < 2) return `${base}_${floor + 1}`
      return 'city2'
    }

    // B) Mega / Extra / New Biomes (mega_<name>_<floor>, extra_<name>_<floor>, new_<name>_<floor>)
    const numberedMatch = mapId.match(/^([a-z0-9]+_[a-z0-9_]+)_(\d+)$/)
    if (numberedMatch) {
      const base = numberedMatch[1]
      const floor = parseInt(numberedMatch[2], 10) || 1
      if (floor < 10) return `${base}_${floor + 1}`
      return 'unified_world'
    }

    // C) Island Deep Dungeon Floors (e.g. continent1_isl2_f4)
    if (mapId.includes('_isl') && mapId.includes('_f')) {
      const parts = mapId.split('_f')
      const base = parts[0]
      const floor = parseInt(parts[1], 10) || 1
      if (floor < 12) return `${base}_f${floor + 1}`
      const contId = mapId.split('_')[0]
      return contId
    }

    // D) 100 Biomes (b1_whispering_woods or b1_whispering_woods_f2 .. f10)
    if (mapId.includes('_f')) {
      const parts = mapId.split('_f')
      const base = parts[0]
      const floor = parseInt(parts[1], 10) || 1
      if (floor < 10) return `${base}_f${floor + 1}`
      return 'unified_world' // Floor 10 Victory returns to world
    }
    const b100 = find100Biome(mapId)
    if (b100) {
      return `${mapId}_f2`
    }

    // D) 100-Floor Dungeon / Masmorra / Catacombs
    if (mapId.startsWith('dungeon') || mapId.startsWith('masmorra') || mapId.startsWith('catacombs')) {
      const floor = parseInt(mapId.replace(/\D/g, ''), 10) || 1
      const prefix = mapId.replace(/\d+/g, '') || 'dungeon'
      if (floor < 100) return `${prefix}${floor + 1}`
      return 'unified_world'
    }

    // E) Endless Tower
    if (mapId.startsWith('endless')) {
      const floor = parseInt(mapId.replace('endless', ''), 10) || 1
      return `endless${floor + 1}`
    }

    // F) Legacy Biomes
    return `${mapId}_f2`
  } else {
    // --- GOING BACK / PREVIOUS FLOOR / RETURN TO WORLD ---
    ...
    if (c2Match) {
      const base = `c2_${c2Match[1]}`
      const floor = parseInt(c2Match[2], 10) || 1
      if (floor > 1) return `${base}_${floor - 1}`
      return 'city2'
    }

    // B) Mega / Extra / New Biomes
    const numberedMatch = mapId.match(/^([a-z0-9]+_[a-z0-9_]+)_(\d+)$/)
    if (numberedMatch) {
      const base = numberedMatch[1]
      const floor = parseInt(numberedMatch[2], 10) || 1
    ...
    // C) Island Deep Dungeon Floors
    if (mapId.includes('_isl') && mapId.includes('_f')) {
      const parts = mapId.split('_f')
      const base = parts[0]
      const floor = parseInt(parts[1], 10) || 1
      if (floor > 1) return `${base}_f${floor - 1}`
      const contId = mapId.split('_')[0]
      return contId
    }
    ...
    if (mapId.includes('_f')) {
      ...
      const base = parts[0]
      const floor = parseInt(parts[1], 10) || 1
      if (floor > 2) return `${base}_f${floor - 1}`
      if (floor === 2) return base // Floor 1 is the base map ID
      return 'unified_world'
    }
    const b100 = find100Biome(mapId)
    if (b100) {
    ...
    if (mapId.startsWith('dungeon') || mapId.startsWith('masmorra') || mapId.startsWith('catacombs')) {
      const floor = parseInt(mapId.replace(/\D/g, ''), 10) || 1
      const prefix = mapId.replace(/\d+/g, '') || 'dungeon'
      if (floor > 1) return `${prefix}${floor - 1}`
      return 'unified_world'
    }
    ...
    if (mapId.startsWith('endless')) {
      const floor = parseInt(mapId.replace('endless', ''), 10) || 1
      if (floor > 1) return `endless${floor - 1}`
      return 'unified_world'
    }
...
export function movePlayer(state: GameState, keys: Set<string>): GameState {
  ...
  // Check portal / stairs tile underfoot
  const ptx = Math.floor((finalX + 16) / 32)
  const pty = Math.floor((finalY + 16) / 32)
  const tileObj = map.tiles[pty]?.[ptx]
  const stepTile = tileObj?.type
  let portalTarget: string | null = null

  // Teleportation ONLY occurs if stepping on a WALKABLE tile (e.g. stairs)
  // Pillars, monuments, solid tiles, or walls NEVER trigger teleportation
  if (state._portalCooldown === undefined || state._portalCooldown <= 0) {
    const isPortalOrStair =
      stepTile === 'portal' ||
      stepTile === 'stairs_up' ||
      stepTile === 'stairs_down' ||
      stepTile === 'forest_portal' ||
      stepTile === 'desert_portal' ||
      stepTile === 'volcano_portal' ||
      stepTile === 'abyss_portal' ||
      stepTile === 'dungeon_portal' ||
      stepTile === 'ocean_portal' ||
      stepTile === 'celestial_portal' ||
      ...
      (stepTile === 'rune_altar' && map.id !== 'temple_blessings') ||
      Boolean(stepTile?.endsWith('_portal'))

    if (isPortalOrStair) {
      const curId = map.id || 'city'
      portalTarget = getPortalTargetForLocation(curId, ptx, pty, stepTile, map)
    }
  }

  // ─── BENÇÃO DO TEMPLO DE BENÇÕES ──────────────────────────────────────────
  if (map.id === 'temple_blessings' && stepTile === 'rune_altar') {
    const blessCooldown = (state as any)._blessCooldown ?? 0
    if (blessCooldown <= 0) {
      let bName = 'Benção Solar (+30% Dano)'
      let bType = 'damage'
      if (ptx === 16 && pty === 24) { bName = 'Benção Solar (+30% Dano)'; bType = 'damage' }
      else if (ptx === 32 && pty === 24) { bName = 'Benção Lunar (+40% XP)'; bType = 'xp' }
      else if (ptx === 24 && pty === 16) { bName = 'Benção da Fortuna (+50% Ouro)'; bType = 'gold' }
      else if (ptx === 24 && pty === 32) { bName = 'Benção Astral (+25% Velocidade & Regen)'; bType = 'astral' }

      updatedPlayer = {
        ...updatedPlayer,
        _activeBlessing: { name: bName, type: bType, expiresAt: Date.now() + 1200000 }
      }
      newNotifs.push({ id: uid('bless'), text: `✨ Recebeu a ${bName}!`, type: 'achievement', timer: 200 })
      playSfx('achievement')
    }
  }
  ...
  if (stepTile === 'iron_ore_node' || stepTile === 'gold_ore_node' ||
    ...
    if (mc <= 0) {
      ...
      if (mined) {
        ...
        const newTiles = [...updatedMap.tiles]
        newTiles[pty] = [...newTiles[pty]]
        newTiles[pty][ptx] = { type: 'stone', walkable: true, transparent: true }
  ...
  if (respawnKeys.length > 0) {
    ...
    for (const k of respawnKeys) {
      ...
      if (left <= 0 && r.mapId === mapMut.id) {
        // restaura tile
        const newTiles = [...mapMut.tiles]
        if (newTiles[r.y]) {
          newTiles[r.y] = [...newTiles[r.y]]
          newTiles[r.y][r.x] = { type: r.tile, walkable: true, transparent: true }
        }
...
export function tickUpdate(state: GameState): GameState {
  ...
  if (s.currentMap && s.currentMap.physicalObjects && s.currentMap.physicalObjects.length > 0) {
    const updatedPhys = updatePhysicsObjects(
      s.currentMap.physicalObjects,
      s.currentMap,
      s.particles,
      s.player ?? undefined,
      s.currentMap.monsters,
      s.damageNumbers
    )
    s.currentMap = { ...s.currentMap, physicalObjects: updatedPhys }
  }

  // Update Dynamic Biome Events
  const evtResult = updateDynamicEvents(s)
  if (evtResult.event) {
    s._activeDynamicEvent = evtResult.event
    s._dynamicEventTimer = evtResult.eventTimer
  } else {
    s._activeDynamicEvent = null
    s._dynamicEventTimer = 0
  }
  if (evtResult.hazardParticles.length > 0) {
    s.particles = [...s.particles, ...evtResult.hazardParticles]
  }
  if (evtResult.hazardSpawns.length > 0 && s.currentMap) {
    s.currentMap = { ...s.currentMap, monsters: [...(s.currentMap.monsters || []), ...evtResult.hazardSpawns] }
  }
  if (evtResult.chatMessage) {
    s = pushNotice(s, evtResult.chatMessage, 'event', 180)
  }
  ...
  if (s.player && s.player.mercenaries && s.player.mercenaries.length > 0 && s.currentMap) {
    const mercResult = updateMercenariesAI(
      s.player.mercenaries,
      s.player,
      s.currentMap.monsters || [],
      s.damageNumbers,
      s.particles
    )
    s.player = { ...mercResult.updatedPlayer, mercenaries: mercResult.mercenaries }
  }
  ...
  // Update ambience & dynamic biome weather
  if (s.currentMap) {
    if (s.currentMap.ambience !== s._lastAmbience) {
      setAmbience(s.currentMap.ambience)
      s._lastAmbience = s.currentMap.ambience
    }

    if (s.currentMap.id === 'unified_world' && s.player) {
      const tileX = Math.floor(s.player.position.x / 32)
      const tileY = Math.floor(s.player.position.y / 32)
      if (s._lastWeatherTileX !== tileX || s._lastWeatherTileY !== tileY) {
        s._lastWeatherTileX = tileX
        s._lastWeatherTileY = tileY
        let closestReg = OPEN_WORLD_REGIONS[0]
        let minD2 = Infinity
        for (let i = 0; i < OPEN_WORLD_REGIONS.length; i++) {
          const reg = OPEN_WORLD_REGIONS[i]
          const dx = tileX - reg.centerX
          const dy = tileY - reg.centerY
        ...
        s._weather = closestReg.weather ?? 'none'
        s._weatherIntensity = 0.65
      }
    }
  }
...
export function changeMap(state: GameState, mapId: string): GameState {
  ...
  if (!newMap.spawnPoints) {
    newMap.spawnPoints = [(newMap as any).playerSpawn || { x: 96, y: 96 }]
  }

  let lastWorldPos = state._lastWorldPos || null
  if (state.currentMap?.id === 'unified_world' && state.player) {
    lastWorldPos = { x: state.player.position.x, y: state.player.position.y }
  }

  let rawSpawn = newMap.spawnPoints[0] || { x: 96, y: 96 }
  if (newMap.id === 'unified_world') {
    if (lastWorldPos) {
      // Spawn 2 tiles south of the portal entered on unified_world
      rawSpawn = { x: lastWorldPos.x, y: lastWorldPos.y + 64 }
    } else {
      rawSpawn = { x: CENTER * 32, y: (CENTER + 3) * 32 }
    }
  }
...
export function saveGame(state: GameState): void {
  ...
  const save = {
    player: state.player,
    mapId: state.currentMap?.id || 'unified_world',
    version: 1,
  }
  try {
...
export function loadGame(): { player: Player; mapId: string } | null {
  try {
    ...
    if (typeof save === 'string') {
      try { save = JSON.parse(save) } catch {}
    }
    if (!save || typeof save !== 'object') return null

    let player: Player | null = save.player || null
    let mapId: string = save.mapId || save.currentMap?.id || 'unified_world'

    if (typeof player === 'string') {
      try { player = JSON.parse(player) } catch {}
    }

    if (!player && (save.name || save.class || save.stats)) {
      player = save as Player
    }

    if (!player || typeof player !== 'object') return null
    return { player: sanitizePlayer(player), mapId }
  } catch (e) {
    console.error('[loadGame] Error parsing save:', e)
    return null
  }
}
