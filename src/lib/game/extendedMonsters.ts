...
/**
 * Registro dinâmico: permite que módulos de conteúdo (ex: megaBiomes) adicionem
 * novos monstros ao registry após o carregamento inicial. Idempotente por id.
 */
export function registerExtendedMonster(def: ExtendedMonsterDef) {
  if (REGISTRY.has(def.id)) return
  EXTENDED_MONSTERS.push(def)
