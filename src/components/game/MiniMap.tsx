...
function MiniMapImpl({ gameState, onToggleWorldMap }: Props) {
  ...
  useEffect(() => {
    ...
    if (base.mapId !== currentMap.id || !base.canvas) {
      ...
      const stepX = Math.max(1, Math.floor(mapW / canvas.width))
      const stepY = Math.max(1, Math.floor(mapH / canvas.height))
      for (let y = 0; y < mapH; y += stepY) {
        for (let x = 0; x < mapW; x += stepX) {
          const tile = currentMap.tiles[y]?.[x]
          if (!tile) continue
          const color = TILE_COLORS[tile.type] || '#1a1a2e'
          bctx.fillStyle = color
          bctx.fillRect(x * scaleX, y * scaleY, scaleX * stepX + 0.5, scaleY * stepY + 0.5)
        }
      }
