...
export function drawTile(
  ...
  switch (type) {
    ...
    case 'portal':
      ...
      break
    case 'forest_portal':
      drawForestPortalTile(ctx, tick)
      break
    case 'desert_portal':
      drawDesertPortalTile(ctx, tick)
      break
    case 'volcano_portal':
      drawVolcanoPortalTile(ctx, tick)
      break
    case 'abyss_portal':
      drawAbyssPortalTile(ctx, tick)
      break
    case 'dungeon_portal':
      drawDungeonPortalTile(ctx, tick)
      break
    case 'ocean_portal':
      drawOceanPortalTile(ctx, tick)
      break
    case 'celestial_portal':
      drawCelestialPortalTile(ctx, tick)
      break
    // ── City tiles ──
    case 'cobblestone':
    ...
    case 'gem_node':
      ...
      break
    case 'crystal_portal':
      drawRuneMonument(ctx, tick, '#04060e', '#1e293b', '#334155', '#38bdf8', '56,189,248')
      break
    // ── Haunted Ruins tiles ────────────────────────────────────────────────
    case 'ruin_floor':
    ...
    case 'cobweb':
      ...
      break
    case 'haunted_portal':
      drawRuneMonument(ctx, tick, '#0a0608', '#2a1a24', '#3f1d38', '#f43f5e', '244,63,94')
      break
    // ── Sky Realm tiles ───────────────────────────────────────────────────
    case 'cloud_floor':
    ...
    case 'sky_void':
      ...
      break
    case 'sky_portal':
      drawRuneMonument(ctx, tick, '#0a1428', '#1e293b', '#334155', '#fef08a', '254,240,138')
      break
    // ── ORE NODES (mineração) ─────────────────────────────────────────
    case 'iron_ore_node': {
    ...
    case 'tower_wall':
      ...
      break
    case 'tower_portal':
      drawRuneMonument(ctx, tick, '#0a0518', '#2e1065', '#3b0764', '#d8b4fe', '216,180,254')
      break
    default:
...
function drawRuneMonument(
  ...
  // Twin carved rune pillars
  ctx.fillStyle = pillarColor
  ctx.fillRect(5, 8, 5, 14)
  ...
  const floatY = Math.sin(tick * 0.08) * 2
  const pulse = 0.35 + Math.sin(tick * 0.12) * 0.25
  ctx.fillStyle = `rgba(${glowRgb}, ${pulse})`
  ctx.fillRect(11, 4, 10, 18)

  // Floating Crystal Orb in center
  ctx.fillStyle = orbColor
  ctx.beginPath()
  ctx.arc(16, 13 + floatY, 4.5, 0, Math.PI * 2)
  ctx.fill()

  // Inner sparkle
  ctx.fillStyle = '#ffffff'
  ctx.beginPath()
  ctx.arc(15, 12 + floatY, 1.5, 0, Math.PI * 2)
  ctx.fill()
}

function drawPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#0f172a', '#475569', '#334155', '#a855f7', '168,85,247')
}

function drawForestPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#06200c', '#15803d', '#166534', '#10b981', '16,185,129')
}

function drawDesertPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#2a1a06', '#d97706', '#b45309', '#facc15', '250,204,21')
}

function drawVolcanoPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#1a0505', '#b91c1c', '#991b1b', '#f97316', '249,115,22')
}

function drawAbyssPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#05020a', '#581c87', '#3b0764', '#c084fc', '192,132,252')
}

function drawDungeonPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#110d18', '#374151', '#1f2937', '#ef4444', '239,68,68')
}

function drawOceanPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#021627', '#0369a1', '#075985', '#38bdf8', '56,189,248')
}

function drawCelestialPortalTile(ctx: CanvasRenderingContext2D, tick: number) {
  drawRuneMonument(ctx, tick, '#0f0c29', '#818cf8', '#4f46e5', '#e0e7ff', '224,231,255')
}
...
function drawClassByMeta(
  ...
  const meta = ALL_100_CLASSES.find(c => c.id === cls) || {
    id: cls,
    name: cls,
    subclass: 'Aventureiro',
    color: '#f59e0b',
    startWeapon: 'wooden_sword',
    icon: '⚔️',
  }

  let primaryColor = meta.color || '#f59e0b'
  const weaponType = meta.startWeapon || 'wooden_sword'
  const subLower = (meta.subclass || '').toLowerCase()
  const nameLower = (meta.name || '').toLowerCase()
  const idLower = (meta.id || '').toLowerCase()

  // ─── SKIN PALETTE OVERRIDES ───
  if (skin === 1) primaryColor = '#d97706' // Aprendiz
  else if (skin === 2) primaryColor = '#7e22ce' // Nobre
  else if (skin === 3) primaryColor = '#0284c7' // Mestre Rúnico
  else if (skin === 4) primaryColor = '#881337' // Voraz
  else if (skin === 5) primaryColor = '#3b0764' // Sombrio
  else if (skin === 6) primaryColor = '#fef08a' // Celestial
  else if (skin === 7) primaryColor = '#dc2626' // Infernal
  else if (skin === 8) primaryColor = '#f59e0b' // Guardião Dourado
  else if (skin === 9) primaryColor = '#1d4ed8' // Caçador de Dragões
  else if (skin === 10) primaryColor = '#a855f7' // Supremo Ancestral

  // Levitação e vibração arcana por classe/id
  const isFloating = subLower.includes('mago') || subLower.includes('celestial') || subLower.includes('necro') ||
                     subLower.includes('espírito') || subLower.includes('tempo') || subLower.includes('astral') ||
                     subLower.includes('invocador') || subLower.includes('seer') || subLower.includes('alma') ||
                     subLower.includes('lich') || idLower.includes('mage') || idLower.includes('necromancer') ||
                     idLower.includes('summoner') || idLower.includes('chronomancer') || idLower.includes('pyromancer') ||
                     idLower.includes('cryomancer') || idLower.includes('stormcaller') || idLower.includes('blood_mage') ||
                     idLower.includes('void') || idLower.includes('phoenix_sage') || idLower.includes('astral_walker') ||
                     idLower.includes('celestial') || idLower.includes('galaxy') || idLower.includes('supreme_god') ||
                     skin === 6 || skin === 10

  const floatOffsetY = isFloating ? Math.sin(animFrame * 0.15) * 3.5 : 0
  const legOffset = isMoving ? Math.sin(animFrame * 0.4) * 3 : 0
  const armOffset = isMoving ? Math.cos(animFrame * 0.4) * 3 : 0

  ctx.save()
  ctx.translate(0, floatOffsetY)

  // ─── 0. CÍRCULO MÁGICO OU SOMBRA NO CHÃO ──────────────────────────────────
  if (isFloating) {
    ctx.fillStyle = 'rgba(0, 0, 0, 0.25)'
    ctx.beginPath(); ctx.ellipse(16, 29 - floatOffsetY * 0.8, 10, 3, 0, 0, Math.PI * 2); ctx.fill()
    ctx.strokeStyle = primaryColor
    ctx.lineWidth = 1
    ctx.beginPath(); ctx.ellipse(16, 29 - floatOffsetY * 0.8, 12 + Math.sin(animFrame * 0.1) * 2, 4, 0, 0, Math.PI * 2); ctx.stroke()
  } else {
    ctx.fillStyle = 'rgba(0, 0, 0, 0.3)'
    ctx.beginPath(); ctx.ellipse(16, 29, 9, 3, 0, 0, Math.PI * 2); ctx.fill()
  }

  // ─── 1. CAPAS / ASAS / EQUIPAMENTOS DAS COSTAS ────────────────────────────
  if (idLower.includes('dragon_rider') || idLower.includes('dragon_slayer')) {
    // Asas de Dragão com Escamas
    ctx.fillStyle = '#991b1b'
    ctx.beginPath(); ctx.moveTo(6, 16); ctx.lineTo(-6, 2); ctx.lineTo(3, 10); ctx.lineTo(-8, 18); ctx.lineTo(8, 18); ctx.fill()
    ctx.beginPath(); ctx.moveTo(26, 16); ctx.lineTo(38, 2); ctx.lineTo(29, 10); ctx.lineTo(40, 18); ctx.lineTo(24, 18); ctx.fill()
  } else if (idLower.includes('phoenix_sage') || idLower.includes('phoenix_knight')) {
    // Asas Flamejantes de Fênix com Faíscas
    ctx.fillStyle = '#f97316'
    ctx.beginPath(); ctx.moveTo(6, 16); ctx.lineTo(-7, -2); ctx.lineTo(2, 8); ctx.lineTo(-4, 18); ctx.lineTo(8, 16); ctx.fill()
    ctx.beginPath(); ctx.moveTo(26, 16); ctx.lineTo(39, -2); ctx.lineTo(30, 8); ctx.lineTo(36, 18); ctx.lineTo(24, 16); ctx.fill()
    ctx.fillStyle = '#fef08a'
    ctx.fillRect(-2 + (animFrame % 4), 4, 2, 2); ctx.fillRect(32 + (animFrame % 4), 4, 2, 2)
  } else if (idLower.includes('valkyrie') || idLower.includes('holy_champion') || idLower.includes('supreme_god') || subLower.includes('valquíria') || subLower.includes('anjo')) {
    // Asas Angelicais Douradas
    ctx.fillStyle = '#ffffff'
    ctx.beginPath(); ctx.moveTo(8, 15); ctx.lineTo(-3, -1); ctx.lineTo(10, 10); ctx.fill()
    ctx.beginPath(); ctx.moveTo(24, 15); ctx.lineTo(35, -1); ctx.lineTo(22, 10); ctx.fill()
    ctx.fillStyle = '#facc15'
    ctx.beginPath(); ctx.moveTo(8, 17); ctx.lineTo(-1, 5); ctx.lineTo(8, 13); ctx.fill()
    ctx.beginPath(); ctx.moveTo(24, 17); ctx.lineTo(33, 5); ctx.lineTo(24, 13); ctx.fill()
  } else if (idLower.includes('vampire_lord') || idLower.includes('blood_reaper') || idLower.includes('blood_mage') || subLower.includes('vampiro')) {
    // Asas de Morcego + Capa de Vampiro
    ctx.fillStyle = '#881337'; ctx.fillRect(4, 10, 24, 18)
    ctx.fillStyle = '#e11d48'
    ctx.beginPath(); ctx.moveTo(4, 12); ctx.lineTo(-4, 4); ctx.lineTo(2, 10); ctx.lineTo(-6, 20); ctx.lineTo(6, 18); ctx.fill()
    ctx.beginPath(); ctx.moveTo(28, 12); ctx.lineTo(36, 4); ctx.lineTo(30, 10); ctx.lineTo(38, 20); ctx.lineTo(26, 18); ctx.fill()
  } else if (idLower.includes('void_slayer') || idLower.includes('void_summoner') || idLower.includes('abyss')) {
    // Portal / Anel de Matéria Negra
    ctx.strokeStyle = '#a855f7'; ctx.lineWidth = 2
    ctx.beginPath(); ctx.ellipse(16, 15, 18, 8, Math.PI / 4, 0, Math.PI * 2); ctx.stroke()
  } else if (idLower.includes('mech_smith') || idLower.includes('pyro_mechanic')) {
    // Mochila Steampunk a Vapor com Escapamentos
    ctx.fillStyle = '#78350f'; ctx.fillRect(6, 12, 20, 14)
    ctx.fillStyle = '#f59e0b'; ctx.fillRect(4, 8, 4, 8); ctx.fillRect(24, 8, 4, 8)
    ctx.fillStyle = 'rgba(241, 245, 249, 0.7)'
    ctx.fillRect(5, 4 - (animFrame % 5), 2, 2); ctx.fillRect(25, 4 - (animFrame % 5), 2, 2)
  } else if (idLower.includes('sand_pharaoh')) {
    // Capa de Linho Egípcia com Scarab
    ctx.fillStyle = '#ca8a04'; ctx.fillRect(5, 12, 22, 16)
    ctx.fillStyle = '#38bdf8'; ctx.beginPath(); ctx.arc(16, 16, 4, 0, Math.PI * 2); ctx.fill()
  } else if (idLower.includes('galaxy') || idLower.includes('astral') || idLower.includes('star')) {
    // Orbitando Anel de Estrelas
    const starAng = animFrame * 0.1
    ctx.fillStyle = '#60a5fa'
    for (let i = 0; i < 3; i++) {
      const sx = 16 + Math.cos(starAng + (i * Math.PI * 2) / 3) * 16
      const sy = 14 + Math.sin(starAng + (i * Math.PI * 2) / 3) * 6
      ctx.fillRect(sx, sy, 3, 3)
    }
  } else if (idLower.includes('titan') || idLower.includes('iron_colossus') || idLower.includes('geomancer') || subLower.includes('titã') || subLower.includes('pedra')) {
    // Placas de Granito
    ctx.fillStyle = '#475569'; ctx.fillRect(5, 11, 22, 16)
    ctx.fillStyle = '#1e293b'; ctx.fillRect(7, 13, 18, 12)
  } else {
    // Capa clássica da classe
    ctx.fillStyle = primaryColor
    ctx.fillRect(6, 12, 20, 15)
  }

  // ─── 2. PERNAS E BOTAS POR SILHUETA ──────────────────────────────────────
  const isHeavyTank = idLower.includes('knight') || idLower.includes('paladin') || idLower.includes('titan') ||
                      idLower.includes('templar') || idLower.includes('colossus') || idLower.includes('dread') ||
                      idLower.includes('glacier') || idLower.includes('berserker') || idLower.includes('golem') ||
                      subLower.includes('cavaleiro') || subLower.includes('paladino') || subLower.includes('guardião') ||
                      subLower.includes('gladiador')
  const bootWidth = isHeavyTank ? 6 : 5

  ctx.fillStyle = isHeavyTank ? '#334155' : '#1e1b18'
  ctx.fillRect(10 + legOffset, 26, bootWidth, 5)
  ctx.fillRect(17 - legOffset, 26, bootWidth, 5)

  ctx.fillStyle = primaryColor
  ctx.fillRect(9 + legOffset, 27, bootWidth + 1, 4)
  ctx.fillRect(17 - legOffset, 27, bootWidth + 1, 4)

  // ─── 3. TRONCO E VESTIMENTA ──────────────────────────────────────────────
  const bodyX = isHeavyTank ? 9 : 10
  const bodyW = isHeavyTank ? 14 : 12

  if (subLower.includes('samurai') || subLower.includes('ronin') || idLower.includes('samurai') || idLower.includes('sun_blade')) {
    // Dō (Armadura Japonesa Tradicional)
    ctx.fillStyle = '#991b1b'; ctx.fillRect(10, 15, 12, 11)
    ctx.fillStyle = '#f59e0b'; ctx.fillRect(10, 17, 12, 2); ctx.fillRect(10, 20, 12, 2)
  } else if (subLower.includes('monge') || subLower.includes('ki') || idLower.includes('monk')) {
    // Kimono de Monge com Faixa Obi
    ctx.fillStyle = '#ea580c'; ctx.fillRect(10, 15, 12, 12)
    ctx.fillStyle = '#fef08a'; ctx.fillRect(10, 21, 12, 2)
  } else if (subLower.includes('mago') || subLower.includes('arquimago') || subLower.includes('feitiç') || subLower.includes('bruxo') || idLower.includes('mage') || idLower.includes('sorcerer') || idLower.includes('warlock') || idLower.includes('witch')) {
    // Túnica Mística Arcana em Camadas
    ctx.fillStyle = '#1e1b4b'; ctx.fillRect(10, 14, 12, 13)
    ctx.fillStyle = primaryColor; ctx.fillRect(14, 14, 4, 13)
    ctx.fillStyle = '#fef08a'; ctx.fillRect(15, 18, 2, 2)
  } else if (subLower.includes('assassino') || subLower.includes('ninja') || subLower.includes('sombra') || idLower.includes('ninja') || idLower.includes('assassin') || idLower.includes('stalker')) {
    // Traje Oculto de Couro Leve com Cinto
    ctx.fillStyle = '#0f172a'; ctx.fillRect(10, 15, 12, 11)
    ctx.fillStyle = '#dc2626'; ctx.fillRect(10, 17, 12, 2)
    ctx.fillStyle = '#cbd5e1'; ctx.fillRect(12, 16, 2, 4); ctx.fillRect(18, 16, 2, 4)
  } else if (isHeavyTank) {
    // Armadura Pesada com Cruz / Blazon
    ctx.fillStyle = '#cbd5e1'; ctx.fillRect(bodyX, 15, bodyW, 11)
    ctx.fillStyle = '#f59e0b'; ctx.fillRect(15, 16, 2, 8); ctx.fillRect(12, 19, 8, 2)
  } else if (idLower.includes('sand_pharaoh')) {
    // Collar Egípcio e Kilt de Linho
    ctx.fillStyle = '#fef08a'; ctx.fillRect(10, 15, 12, 5)
    ctx.fillStyle = '#0284c7'; ctx.fillRect(10, 20, 12, 6)
  } else if (idLower.includes('plague_doctor')) {
    // Sobretudo Médico de Couro Negro
    ctx.fillStyle = '#1e293b'; ctx.fillRect(10, 14, 12, 13)
    ctx.fillStyle = '#94a3b8'; ctx.fillRect(15, 15, 2, 10)
  } else {
    // Túnica / Peitoral Padrão
    ctx.fillStyle = primaryColor; ctx.fillRect(bodyX, 15, bodyW, 11)
    ctx.fillStyle = '#cbd5e1'; ctx.fillRect(bodyX + 2, 16, bodyW - 4, 8)
  }

  // Hombreiras metálicas / plumas
  ctx.fillStyle = isHeavyTank ? '#f59e0b' : '#e2e8f0'
  const padW = isHeavyTank ? 5 : 4
  ctx.fillRect(bodyX - padW + 1, 14, padW, 5)
  ctx.fillRect(bodyX + bodyW - 1, 14, padW, 5)

  // ─── 4. CABEÇA E TOM DE PELE ─────────────────────────────────────────────
  let skinColor = '#f3c8a8'
  if (idLower.includes('vampire') || idLower.includes('blood') || idLower.includes('death') || subLower.includes('vampiro')) skinColor = '#e2e8f0'
  else if (idLower.includes('dread') || idLower.includes('shadow') || idLower.includes('abyss') || subLower.includes('necro') || subLower.includes('morte')) skinColor = '#94a3b8'
  else if (idLower.includes('fire') || idLower.includes('pyro') || idLower.includes('inferno') || subLower.includes('infernal')) skinColor = '#fca5a5'
  else if (idLower.includes('frost') || idLower.includes('ice') || idLower.includes('glacier') || subLower.includes('gelo')) skinColor = '#a5f3fc'
  else if (idLower.includes('druid') || idLower.includes('shaman') || idLower.includes('poison') || subLower.includes('orco')) skinColor = '#86efac'

  ctx.fillStyle = skinColor
  ctx.fillRect(11, 6, 10, 9)

  // Olhos reluzentes
  ctx.fillStyle = (idLower.includes('vampire') || idLower.includes('blood') || subLower.includes('assassino')) ? '#ef4444' :
                  (idLower.includes('frost') || idLower.includes('ice')) ? '#38bdf8' : '#0f172a'
  if (direction === 'down' || direction === 'right') ctx.fillRect(17, 9, 2, 2)
  if (direction === 'down' || direction === 'left') ctx.fillRect(13, 9, 2, 2)

  // ─── 5. CAPACETES / CHAPÉUS / COROAS / MÁSCARAS ────────────────────────────
  if (subLower.includes('mago') || subLower.includes('arquimago') || subLower.includes('bruxo') || subLower.includes('feitiç') || idLower.includes('mage') || idLower.includes('witch') || idLower.includes('sorcerer')) {
    // Chapéu de Mago Puntiagudo
    ctx.fillStyle = primaryColor
    ctx.beginPath(); ctx.moveTo(4, 7); ctx.lineTo(28, 7); ctx.lineTo(16, -4); ctx.closePath(); ctx.fill()
    ctx.fillStyle = '#facc15'
    ctx.fillRect(9, 6, 14, 2)
    ctx.fillRect(15, 1, 2, 2)
  } else if (subLower.includes('cavaleiro') || subLower.includes('paladino') || subLower.includes('guardião') || subLower.includes('gladiador') || idLower.includes('knight') || idLower.includes('paladin')) {
    // Capacete de Cavaleiro com Penacho Vermelho
    ctx.fillStyle = '#475569'; ctx.fillRect(10, 3, 12, 11)
    ctx.fillStyle = '#94a3b8'; ctx.fillRect(11, 7, 10, 2)
    ctx.fillStyle = '#dc2626'; ctx.fillRect(15, -3, 2, 7); ctx.fillRect(13, -2, 6, 2)
  } else if (subLower.includes('assassino') || subLower.includes('ninja') || subLower.includes('gatuno') || idLower.includes('ninja') || idLower.includes('assassin')) {
    // Capuz das Sombras + Máscara Facial com Olhos Brilhantes
    ctx.fillStyle = '#0f172a'; ctx.fillRect(10, 3, 12, 11)
    ctx.fillStyle = '#dc2626'; ctx.fillRect(12, 7, 3, 2); ctx.fillRect(17, 7, 3, 2)
  } else if (subLower.includes('arqueiro') || subLower.includes('caçador') || idLower.includes('archer') || idLower.includes('ranger')) {
    // Capuz do Arqueiro com Pena Dourada
    ctx.fillStyle = '#15803d'; ctx.fillRect(10, 3, 12, 6)
    ctx.fillStyle = '#f59e0b'
    ctx.beginPath(); ctx.moveTo(21, 5); ctx.lineTo(27, -3); ctx.lineTo(22, 6); ctx.closePath(); ctx.fill()
  } else if (subLower.includes('samurai') || subLower.includes('ronin') || subLower.includes('shogun') || idLower.includes('samurai') || idLower.includes('sun_blade')) {
    // Capacete Kabuto com Lua Crescente (Kuwagata)
    ctx.fillStyle = '#1e293b'; ctx.fillRect(9, 2, 14, 6)
    ctx.fillStyle = '#f59e0b'; ctx.beginPath(); ctx.arc(16, 1, 6, Math.PI, Math.PI * 2); ctx.fill()
  } else if (subLower.includes('druida') || subLower.includes('floresta') || subLower.includes('xamã') || idLower.includes('druid') || idLower.includes('shaman')) {
    // Chifres de Cervo + Coroa de Folhas
    ctx.fillStyle = '#78350f'; ctx.fillRect(8, -3, 2, 8); ctx.fillRect(22, -3, 2, 8); ctx.fillRect(6, -1, 3, 2); ctx.fillRect(23, -1, 3, 2)
    ctx.fillStyle = '#22c55e'; ctx.fillRect(11, 4, 10, 2)
  } else if (subLower.includes('bardo') || subLower.includes('trovador') || idLower.includes('bard')) {
    // Boina de Bardo com Pena Cantante e Nota Musical Flutuante
    ctx.fillStyle = primaryColor; ctx.fillRect(9, 2, 14, 4)
    ctx.fillStyle = '#fef08a'; ctx.fillRect(15, -2, 2, 5)
    ctx.font = '8px sans-serif'
    ctx.fillText('♪', 2 + Math.sin(animFrame * 0.2) * 2, 8)
  } else if (subLower.includes('cronomante') || subLower.includes('tempo') || idLower.includes('chronomancer')) {
    // Elmo Relógio de Engrenagens
    ctx.fillStyle = '#d97706'; ctx.beginPath(); ctx.arc(16, 3, 5, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = '#fef08a'; ctx.fillRect(15, 2, 2, 3)
  } else if (subLower.includes('necro') || subLower.includes('morte') || idLower.includes('necromancer')) {
    // Capuz Sombrio do Necromante
    ctx.fillStyle = '#1e1b4b'; ctx.fillRect(9, 2, 14, 10)
    ctx.fillStyle = '#a855f7'; ctx.fillRect(12, 8, 2, 2); ctx.fillRect(18, 8, 2, 2)
  } else if (idLower.includes('plague_doctor')) {
    // Máscara de Bico de Corvo + Fedora
    ctx.fillStyle = '#0f172a'; ctx.fillRect(9, 1, 14, 6); ctx.fillRect(6, 6, 20, 2)
    ctx.fillStyle = '#f8fafc'; ctx.beginPath(); ctx.moveTo(16, 9); ctx.lineTo(22, 13); ctx.lineTo(16, 12); ctx.fill()
  } else if (idLower.includes('sand_pharaoh')) {
    // Nemes Pharaônico com Naja
    ctx.fillStyle = '#ca8a04'; ctx.fillRect(8, 2, 16, 6); ctx.fillRect(8, 5, 3, 7); ctx.fillRect(21, 5, 3, 7)
    ctx.fillStyle = '#ef4444'; ctx.fillRect(15, 1, 2, 3)
  } else if (idLower.includes('mech_smith') || idLower.includes('pyro_mechanic') || subLower.includes('engenheiro') || subLower.includes('alquimista')) {
    // Óculos Steampunk na Testa
    ctx.fillStyle = '#78350f'; ctx.fillRect(10, 3, 12, 3)
    ctx.fillStyle = '#f59e0b'; ctx.beginPath(); ctx.arc(13, 4, 3, 0, Math.PI * 2); ctx.fill()
    ctx.beginPath(); ctx.arc(19, 4, 3, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = '#38bdf8'; ctx.fillRect(13, 4, 1, 1); ctx.fillRect(19, 4, 1, 1)
  } else if (idLower.includes('demon_hunter')) {
    // Chapéu Van Helsing com Cruz de Prata
    ctx.fillStyle = '#0f172a'; ctx.fillRect(9, 2, 14, 5); ctx.fillRect(5, 6, 22, 2)
    ctx.fillStyle = '#e2e8f0'; ctx.fillRect(15, 3, 2, 3)
  } else if (idLower.includes('dragon_rider') || idLower.includes('dragon_slayer')) {
    // Capacete de Crânio de Dragão
    ctx.fillStyle = '#7f1d1d'; ctx.fillRect(9, 2, 14, 8)
    ctx.fillStyle = '#fef08a'; ctx.fillRect(7, 0, 3, 5); ctx.fillRect(22, 0, 3, 5)
    ctx.fillStyle = '#ef4444'; ctx.fillRect(12, 7, 2, 2); ctx.fillRect(18, 7, 2, 2)
  } else if (idLower.includes('phoenix')) {
    // Coroa de Penas Flamejantes
    ctx.fillStyle = '#f97316'
    ctx.beginPath(); ctx.moveTo(10, 6); ctx.lineTo(12, -2); ctx.lineTo(14, 6); ctx.fill()
    ctx.beginPath(); ctx.moveTo(18, 6); ctx.lineTo(20, -2); ctx.lineTo(22, 6); ctx.fill()
    ctx.beginPath(); ctx.moveTo(14, 6); ctx.lineTo(16, -5); ctx.lineTo(18, 6); ctx.fill()
  } else if (idLower.includes('frost') || idLower.includes('ice') || idLower.includes('glacier') || subLower.includes('criomante')) {
    // Coroa de Cristais de Gelo
    ctx.fillStyle = '#38bdf8'
    ctx.beginPath(); ctx.moveTo(11, 6); ctx.lineTo(13, -2); ctx.lineTo(15, 6); ctx.fill()
    ctx.beginPath(); ctx.moveTo(17, 6); ctx.lineTo(19, -2); ctx.lineTo(21, 6); ctx.fill()
    ctx.beginPath(); ctx.moveTo(14, 6); ctx.lineTo(16, -5); ctx.lineTo(18, 6); ctx.fill()
  } else if (idLower.includes('sun') || idLower.includes('solar') || idLower.includes('holy_champion')) {
    // Coroa Solar de Raios Dourados
    ctx.strokeStyle = '#fef08a'; ctx.lineWidth = 2
    ctx.beginPath(); ctx.arc(16, 3, 7, 0, Math.PI * 2); ctx.stroke()
  } else {
    // Elmo metálico básico
    ctx.fillStyle = '#475569'; ctx.fillRect(10, 3, 12, 5)
  }
  ...
  const wx = 23 + armOffset
  const wy = 15

  if (weaponType.includes('bow') || subLower.includes('arqueiro') || idLower.includes('archer') || idLower.includes('ranger') || idLower.includes('stalker')) {
    // Arco Recurvo com Corda
    ctx.strokeStyle = '#854d0e'; ctx.lineWidth = 2.5
    ctx.beginPath(); ctx.arc(wx + 4, wy + 2, 11, -Math.PI/2, Math.PI/2); ctx.stroke()
    ctx.strokeStyle = '#fff'; ctx.lineWidth = 1
    ctx.beginPath(); ctx.moveTo(wx + 4, wy - 9); ctx.lineTo(wx + 4, wy + 13); ctx.stroke()
  } else if (weaponType.includes('staff') || weaponType.includes('wand') || subLower.includes('mago') || subLower.includes('druida') || subLower.includes('alquimista') || idLower.includes('mage') || idLower.includes('shaman') || idLower.includes('priest') || idLower.includes('oracle')) {
    // Cajado Arcano com Joia Pulsante
    ctx.fillStyle = '#78350f'; ctx.fillRect(wx + 2, wy - 11, 2, 24)
    ctx.fillStyle = primaryColor
    ctx.beginPath(); ctx.arc(wx + 3, wy - 12, 4 + Math.sin(animFrame * 0.2), 0, Math.PI * 2); ctx.fill()
  } else if (weaponType.includes('scythe') || subLower.includes('necro') || subLower.includes('ceifador') || idLower.includes('reaper') || idLower.includes('necromancer')) {
    // Foice Gigante do Necromante/Ceifador
    ctx.fillStyle = '#334155'; ctx.fillRect(wx + 2, wy - 14, 2, 28)
    ctx.fillStyle = '#e2e8f0'
    ctx.beginPath(); ctx.moveTo(wx + 3, wy - 14); ctx.lineTo(wx + 17, wy - 8); ctx.lineTo(wx + 3, wy - 2); ctx.closePath(); ctx.fill()
  } else if (idLower.includes('ocean_king') || idLower.includes('sea_phantom')) {
    // Tridente das Profundezas
    ctx.fillStyle = '#0284c7'; ctx.fillRect(wx + 2, wy - 13, 2, 26)
    ctx.fillRect(wx - 2, wy - 13, 10, 2)
    ctx.fillRect(wx - 2, wy - 18, 2, 5); ctx.fillRect(wx + 2, wy - 18, 2, 5); ctx.fillRect(wx + 6, wy - 18, 2, 5)
  } else if (subLower.includes('pistoleiro') || idLower.includes('demon_hunter') || idLower.includes('gunner') || idLower.includes('mech_smith')) {
    // Pistolas / Trabucos Duplos
    ctx.fillStyle = '#334155'; ctx.fillRect(wx + 1, wy - 4, 10, 3); ctx.fillRect(wx + 1, wy - 1, 3, 5)
  } else if (subLower.includes('samurai') || idLower.includes('samurai') || idLower.includes('blade_dancer') || idLower.includes('ninja')) {
    // Katana Afiada de Aço Radiante
    ctx.fillStyle = '#f8fafc'; ctx.fillRect(wx + 2, wy - 12, 2, 18)
    ctx.fillStyle = '#ea580c'; ctx.fillRect(wx - 1, wy + 2, 7, 2)
  } else if (subLower.includes('assassino') || idLower.includes('assassin')) {
    // Adagas Duplas da Sombra
    ctx.fillStyle = '#cbd5e1'; ctx.fillRect(wx + 2, wy - 6, 2, 12)
    ctx.fillRect(5 - armOffset, wy - 6, 2, 12)
  } else if (subLower.includes('bardo') || idLower.includes('bard')) {
    // Alaúde / Harpa Cantante
    ctx.fillStyle = '#b45309'; ctx.beginPath(); ctx.ellipse(wx + 3, wy + 2, 5, 7, 0, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = '#fef08a'; ctx.fillRect(wx + 2, wy - 6, 2, 8)
  } else {
    // Espada Longa / Montante
    ctx.fillStyle = '#cbd5e1'; ctx.fillRect(wx + 2, wy - 10, 2, 16)
    ctx.fillStyle = '#f59e0b'; ctx.fillRect(wx - 1, wy + 3, 7, 2)
  }

  // ─── 7. EFEITO ESPECÍFICO DE ATAQUE ──────────────────────────────────────
  if (isAttacking) {
    const fxFrame = animFrame % 34
    const radius = 10 + (fxFrame % 10) * 2
    ctx.save()
    ctx.strokeStyle = primaryColor
    ctx.lineWidth = 2.5
    ctx.beginPath(); ctx.arc(16, 16, radius, 0, Math.PI * 2); ctx.stroke()

    ctx.fillStyle = '#ffffff'
    for (let i = 0; i < 6; i++) {
      const angle = ((fxFrame + i * 5) * Math.PI) / 15
      const px = 16 + Math.cos(angle) * (radius + 4)
      const py = 16 + Math.sin(angle) * (radius + 4)
...
export function drawCharacter(
  ...
  drawClassByMeta(ctx, cls, direction, isMoving, isAttacking, animFrame, skin)
...
const AURAS: SkinAura[] = ['gold', 'shadow', 'fire', 'ice', 'nature', 'arcane']
const ACCESSORIES: SkinAccessory[] = [
  'cape-red', 'cape-blue', 'cape-purple', 'cape-gold',
  'halo', 'crown', 'horns', 'mask', 'mohawk',
  'pauldrons-gold', 'pauldrons-bone', 'leaf-crown'
]
...
export function getSkinStyle(cls: CharacterClass, skin: number): SkinStyle {
  if (skin === 0) return {}
  const aura = AURAS[skin % AURAS.length]
  const accessory = ACCESSORIES[skin % ACCESSORIES.length]
...
export function drawSnowyMountainTile(ctx: CanvasRenderingContext2D, type: TileType, tick: number) {
  switch (type) {
    ...
    case 'mountain_portal':
      drawRuneMonument(ctx, tick, '#b8c4d0', '#475569', '#334155', '#7dd3fc', '125,211,252')
      break
...
export function drawAncientRuinsTile(ctx: CanvasRenderingContext2D, type: TileType, tick: number) {
  switch (type) {
    ...
    case 'ancient_tile':
      ...
      break
    case 'ruins_portal':
      drawRuneMonument(ctx, tick, '#1a1612', '#451a03', '#78350f', '#fde047', '253,224,71')
      break
    case 'rune_stone':
      ctx.fillStyle = '#2a2418'; ctx.fillRect(0,0,S,S)
